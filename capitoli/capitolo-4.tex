...
Il sistema richiede obbligatoriamente dall'utente le informazioni sopra descritte.
...
Ogni ticket possiede un identificativo univoco e specifica un compito da svolgere all'interno del progetto ben preciso; citiamo per esempio:
\begin{itemize}
...
In questo caso, essendo il progetto portato avanti da una persona singola in maniera incrementale, il flusso comprendeva gli stati:
\begin{enumerate}
	\item \bd{TO DO}: attività da svolgere;
	\item \bd{IN PROGRESS}: attività iniziato da completare;
...
Essa spiega nel modo più specifico possibile, il motivo per cui il ticket è stato creato.
...
Successivamente alla user story, devono essere inseriti gli artefatti tecnici, ovvero il dettaglio delle caratteristiche.
...
Ad ogni requisito viene assegnato il codice identificativo univoco:
	...
	Inoltre, tutti i requisiti individuati e scelti dalla candidata, presentano nella tabella come fonte ``Interno'' e come grado di urgenza ``0'' poiché il loro soddisfacimento non è strettamente necessario.
